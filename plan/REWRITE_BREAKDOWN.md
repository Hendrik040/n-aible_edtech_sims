...
| Key | Meaning |
| --- | --- |
| `depends_on` | List of upstream `phase-X.Y` ids — ticket is not "ready" until every upstream PR is merged into `rewrite/agent-sdk`. |
...
Target branch for every PR in this track: **`rewrite/agent-sdk`** (not
`production-v3`).
