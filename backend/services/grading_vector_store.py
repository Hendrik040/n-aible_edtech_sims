...
class GradingVectorStore:
    ...
    async def search_grading_materials(
        ...
        """
        try:
            debug_log(f"[VECTOR_STORE] Searching grading materials for simulation {simulation_id}")
            ...
            debug_log(f"[VECTOR_STORE] Found {len(formatted_results)} relevant grading materials")
            return formatted_results
            
        except Exception as e:
            debug_log(f"[VECTOR_STORE] Search error: {str(e)}")
            return []
...
def create_search_grading_materials_tool(vector_store: GradingVectorStore) -> BaseTool:
    ...
    async def search_grading_materials_func(simulation_id: int, query: str) -> str:
        ...
        try:
            ...
            debug_log(f"[SEARCH_TOOL] Error: {str(e)}")
