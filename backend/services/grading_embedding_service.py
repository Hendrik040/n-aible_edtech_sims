...
class GradingEmbeddingService:
    ...
    async def process_grading_material(
        ...
        """
        debug_log(f"[GRADING_EMBEDDING] Processing material {material_id}: {filename}")
        ...
        try:
            ...
            debug_log(f"[GRADING_EMBEDDING] Created {len(chunks)} semantic chunks")
            ...
            debug_log(f"[GRADING_EMBEDDING] Successfully processed {len(stored_chunks)} chunks")
            return {
                "status": "success",
                "chunks_created": len(stored_chunks),
            ...
            debug_log(f"[GRADING_EMBEDDING] Error processing material {material_id}: {str(e)}")
    ...
    async def retrieve_grading_context(
        ...
        """
        debug_log(f"[GRADING_RETRIEVAL] Retrieving context for simulation {simulation_id}")
        
        try:
            # Get all chunks for the simulation
            chunks = self._get_simulation_chunks(simulation_id)
            if not chunks:
                debug_log(f"[GRADING_RETRIEVAL] No chunks found for simulation {simulation_id}")
                return []
            ...
            debug_log(f"[GRADING_RETRIEVAL] Retrieved {len(top_chunks)} relevant chunks")
            return top_chunks
            
        except Exception as e:
            debug_log(f"[GRADING_RETRIEVAL] Error retrieving context: {str(e)}")
            return []
    ...
    async def _get_embedding_async(self, text: str, cache_key: str) -> List[float]:
        ...
        try:
            ...
            debug_log(f"[GRADING_EMBEDDING] Failed to get embedding: {str(e)}")
    ...
    async def _store_chunks(self, material_id: int, chunk_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            db.rollback()
            debug_log(f"[GRADING_EMBEDDING] Error storing chunks: {str(e)}")
            raise
        finally:
            db.close()
    ...
    def _get_simulation_chunks(self, simulation_id: int) -> List[Dict]:
        ...
        try:
            ...
            debug_log(f"[GRADING_RETRIEVAL] Error getting simulation chunks: {str(e)}")
            return []
        finally:
            db.close()
    ...
    def _update_material_status(
        ...
        try:
            ...
            db.rollback()
            debug_log(f"[GRADING_EMBEDDING] Error updating material status: {str(e)}")
        finally:
            db.close()
