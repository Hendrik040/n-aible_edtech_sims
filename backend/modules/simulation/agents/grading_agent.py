...
class GradingAgent:
    ...
    def _get_overall_grading_system_prompt(self, strictness_level: int = 3) -> str:
        ...
        return prompt_loader.render(
            "grading",
            section="OVERALL_GRADING_SYSTEM",
            strictness_level=str(strictness_level),
            strictness_clause=strictness_clause,
        )
...
Apply the same scoring standards as scene grading. The overall score must reflect demonstrated performance across scenes, not an upward-adjusted interpretation of it."""
    ...
    async def validate_goal_achievement(self,
        ...
        messages = [
            SystemMessage(content=prompt_loader.render(
                "grading",
                section="GOAL_VALIDATION_SYSTEM",
                threshold_pct=str(threshold_pct),
            )),
            HumanMessage(content=prompt_loader.render(
                "grading",
                section="GOAL_VALIDATION_USER",
                scene_goal=scene_goal,
                scene_description=scene_description,
                current_attempts=str(current_attempts),
